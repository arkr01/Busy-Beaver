...
class TuringMachine:
    ...
    def __init__(self, transitions, start_state='q0', accept_state='qa', reject_state='qr',
                 blank_symbol=''):
    ...
    def run(self, input_):
        ...
        keyword and generators, and hopefully never, ever write something like:
        ...
        [1] https://www.jeffknupp.com/blog/2013/04/07/improve-your-python-yield-and-generators
        -explained/
        ...
        while True:
            ...
            # !<<<<<<<<< MODIFY ABOVE >>>>>>>>>>>>
                ...
                assert direction in 'LR', 'L (left) and R (right) are the only correct directions' \
                                          'to move.'
                logging.warning('An attempt to move left from the leftmost cell! The machine '
                                'stays put.')

    def accepts(self, input_, step_limit=100):
        action = list(islice(self.run(input_), step_limit))[-1][0]
        self.debug(input_, step_limit, colored=True)
